...
impl<'a, T: GuestStagePagingMode> FinalizedVm<'a, T> {
    ...
    fn handle_ecall(&self, msg: SbiMessage, active_vcpu: &mut ActiveVmCpu<T>) -> EcallAction {
        match msg {
            ...
            SbiMessage::Rcode(_) => Err(EcallError::Sbi(SbiError::NotSupported)).into(),
        }
    }
    ...
    fn bind_vcpu_begin(&self, vcpu_id: u64, interrupt_file: ImsicFileId) -> EcallResult<()> {
        self.vm()
            .vcpus
            .get_vcpu(vcpu_id)
    ...
    fn bind_vcpu_end(&self, vcpu_id: u64) -> EcallResult<()> {
        self.vm()
            .vcpus
            .get_vcpu(vcpu_id)
